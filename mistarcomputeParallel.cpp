...
int main (int argc, char *argv[]) {
    ...
    string usage=string(""+string(argv[0])+"  [mode]"+
			...
			"\t\t"+"pairdivcompute  To compute pairwise divergence\n"+
			"\t\t"+"pairdiff        To compute pairwise nucleotide differences\n"+
			"\t\t"+"nj              Compute neighbor-joining tree\n"+
			"\t\t"+"dstat           To compute pairwise D-statistics\n");
    ...
    if( string(argv[1]) == "pairdivcompute"){
	mode='d';	
	usage=string(string(argv[0])+" "+string(argv[1])+" [options] [mistar file (zip or plain)]\n\n");
    }else{
